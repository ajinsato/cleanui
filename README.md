...
Linux **不会在无任何交互的前提下**把你的帐号变成永久 root；可选做法如下：

| 方式 | 说明 |
|------|------|
| **`cleanui --sudo-warmup`** | 启动界面之前在终端执行一次 **`sudo -v`**，刷新本机 **sudo 时间戳**（常见默认约 **15 分钟**）。这段时间内，清理步骤里的 **`sudo apt-get` / `sudo journalctl`** 等通常**不再需要重复输入密码**。 |
| **`cleanui-sudo`** | 与上一行等价（pip 安装后多出来的入口脚本，便于把桌面菜单 `Exec` 改成始终预热凭证）。 |
| **终端里先 `sudo -v` 再 `cleanui`** | 效果同上，手动刷新凭证。 |
| **`sudoers` 细粒度免密** | 可由管理员配置 **仅对固定路径命令** `NOPASSWD`（风险高，需自行审计）。不推荐对整条 shell 或整个 `sudo` 放行。 |

从**桌面图标**启动时，通常没有 TTY，若 **`sudo -v` 无法弹出密码框**，请改用**终端**执行：`cleanui --sudo-warmup` 或先 `sudo -v` 再点图标。
